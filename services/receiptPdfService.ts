// services/receiptPdfService.ts
// Pure Vector PDF 1.4 generator & fiscal quietanza engine (F10, F11, F12)
// Compatible with both Node.js (Buffer) and browser runtime (Uint8Array, Blob)

import { RentReceipt, Property, Tenant, Landlord, RentalRecord } from '../types';
 ...
 */
export function evaluateStampDuty(
  taxRegime: 'CEDOLARE_SECCA' | 'ORDINARIO' | 'ESENTE' = 'CEDOLARE_SECCA',
 ...
 */
export function allocateSequentialReceiptNumber(
  existingReceipts: RentReceipt[] = [],
  targetFiscalYear: number
): { receiptNumber: number; fiscalYear: number; formattedNumber: string; id: string } {
...
export interface CreateReceiptParams {
  ...
  paymentRecord?: RentalRecord | Partial<RentalRecord> | null;
...
export function createRentReceipt({
  ...
  notes = ''
}: CreateReceiptParams): RentReceipt {
  const issueDate = new Date().toISOString().split('T')[0];
  const fiscalYear = paymentRecord?.year || new Date().getFullYear();
  ...
  return {
    ...
    landlordName: landlord?.name || 'Locatore',
    landlordTaxCode: landlord?.taxCode || '',
    landlordAddress: landlord?.address || '',
    competencePeriod: competencePeriod || `Competenza ${paymentRecord?.month !== undefined ? paymentRecord.month + 1 : 1}/${fiscalYear}`,
    rentAmount,
    expensesAmount,
    totalAmount,
    taxRegime,
...
/**
 * Generates a clean, compliant Vector PDF 1.4 document.
 * Zero external npm dependencies. Compatible with Node.js and browser.
 */
export function generateReceiptPdf(receipt: RentReceipt): Uint8Array {
  const width = 595.28;  // A4 portrait width in points
  const height = 841.89; // A4 portrait height in points

  const stamp = evaluateStampDuty(receipt.taxRegime, receipt.totalAmount);
  ...
  const streamLines = [
    ...
    '400 0 Td',
    `(${escapePdfText(`Euro ${(receipt.rentAmount || 0).toFixed(2)}`)}) Tj`,
    'ET',
    ...
    '400 0 Td',
    `(${escapePdfText(`Euro ${(receipt.expensesAmount || 0).toFixed(2)}`)}) Tj`,
    'ET',
    ...
    '380 0 Td',
    `(${escapePdfText(`Euro ${(receipt.totalAmount || 0).toFixed(2)}`)}) Tj`,
    'ET',
...
/**
 * Triggers a browser download of the generated PDF receipt.
 */
export function downloadReceiptPdf(receipt: RentReceipt): void {
  const pdfBytes = generateReceiptPdf(receipt);
  const blob = new Blob([pdfBytes as unknown as BlobPart], { type: 'application/pdf' });
  const url = URL.createObjectURL(blob);
  const safeFilename = `Ricevuta_${receipt.formattedNumber.replace('/', '_')}.pdf`;
