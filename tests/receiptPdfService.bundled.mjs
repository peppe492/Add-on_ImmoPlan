...
function evaluateStampDuty(taxRegime = "CEDOLARE_SECCA", totalAmount = 0) {
...
function allocateSequentialReceiptNumber(existingReceipts = [], targetFiscalYear) {
...
function createRentReceipt({
  ...
  const fiscalYear = paymentRecord?.year || (/* @__PURE__ */ new Date()).getFullYear();
  ...
  return {
    ...
    competencePeriod: competencePeriod || `Competenza ${paymentRecord?.month !== void 0 ? paymentRecord.month + 1 : 1}/${fiscalYear}`,
    rentAmount,
    expensesAmount,
    totalAmount,
    taxRegime,
