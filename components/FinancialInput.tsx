    ...
    <div className="mt-3 pt-3 border-t border-slate-100">
        ...
        <div className="space-y-2">
              ...
              <div key={i} className={`flex items-center justify-between bg-slate-50 p-2 rounded-lg border ${isOverdraft ? 'border-rose-300' : 'border-slate-200'}`}>
                 <div className="flex items-center gap-2 flex-1 min-w-0">
                     <select 
                      ...
                      {isOverdraft && <span className="text-[8px] text-rose-500 font-black" title="Fondi insufficienti">⚠️</span>}
                 </div>
                 <div className="flex items-center gap-2">
                     <input 
                        type="number" 
          ...
          <button 
            ...
            className="w-full py-2.5 bg-white border border-dashed border-slate-300 text-slate-500 rounded-lg text-[10px] font-bold uppercase hover:bg-slate-50 hover:border-slate-400 transition-all flex justify-center items-center gap-1"
          >
            <span>✨</span> Gestisci Pagamenti
          </button>
        </div>
             ...
             <div className="col-span-7 md:col-span-5">
                 ...
                 {!isOpen && (
                     <div className="flex items-center gap-2 text-[10px] text-slate-400 font-medium">
                         {cost.paymentDate && <span>📅 {new Date(cost.paymentDate).toLocaleDateString()}</span>}
                         {cost.attachment && <span>📎 1 All.</span>}
                     </div>
                 )}
             </div>
      ...
      <div className="bg-white p-3 lg:p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-4 sticky top-0 z-40">
          <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-3">
                 <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-xl shrink-0">💾</div>
                 <div className="relative">
                     {isRenaming ? (
                         <input 
                            autoFocus
                             ...
                             <div className="max-h-60 overflow-y-auto custom-scrollbar">
                                 {scenarios.length === 0 ? (
                                     ...
                                     <div key={s.id} className="flex justify-between items-center p-2 hover:bg-slate-50 rounded-lg group cursor-pointer" onClick={() => { onLoadScenario(s.id); setShowScenarioMenu(false); }}>
                                         ...
                                         <button onClick={(e) => { e.stopPropagation(); onDeleteScenario(s.id); }} className="text-slate-300 hover:text-rose-500 px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">🗑️</button>
                                     </div>
                                 ))}
                             </div>
                                 ...
                                 {activeScenario && (
                                     <div className="grid grid-cols-2 gap-2 mb-3">
                                         ...
                                         <button 
                                            ...
                                            className="bg-slate-50 text-slate-600 text-[9px] font-black uppercase py-2 rounded-lg border border-slate-100 hover:bg-slate-100 transition-all flex items-center justify-center gap-1"
                                         >
                                             <span>👯</span> Duplica
                                         </button>
                                     </div>
      ...
      <div className="animate-fade-in px-1">
          ...
          {activeTab === 'PURCHASE' && (
              <div className="bg-white p-4 lg:p-8 rounded-[2rem] shadow-soft border border-slate-200">
                  <div className="flex items-center gap-3 mb-8 px-1">
                      <span className="text-2xl bg-indigo-100 text-indigo-600 p-3 rounded-2xl">💰</span>
                      <div>
          ...
          {activeTab === 'RENOVATION' && (
              <div className="space-y-6">
                  <div className="bg-white p-4 lg:p-8 rounded-[2rem] shadow-soft border border-slate-200">
                      <div className="flex items-center gap-3 mb-8 px-1">
                          <span className="text-2xl bg-amber-100 text-amber-600 p-3 rounded-2xl">🔨</span>
                          <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                           <h4 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><span>📐</span> Tecnici & Design</h4>
                           <PayableCostInput 
                      ...
                      <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                           <h4 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><span>🛡️</span> Riserva Imprevisti</h4>
