...
import { decode, encode, decodeAudioData, getApiKey } from '../services/geminiService';
  ...
  const startSession = async () => {
    ...
    const sessionPromise = ai.live.connect({
      ...
      config: {
        ...
        systemInstruction: `Sei l'assistente vocale di ImmoPlan AI. Aiuta ${owner1Name} e ${owner2Name} con i loro dubbi immobiliari. Sii cordiale e professionale. Dati correnti del loro progetto: ${dataContext}`,
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] bg-white rounded-3xl shadow-soft border border-slate-100 p-8">
      <div className={`w-32 h-32 rounded-full flex items-center justify-center text-4xl mb-8 transition-all duration-500 ${isActive ? 'bg-brand-500 text-white scale-110 shadow-glow' : 'bg-slate-100 text-slate-400'}`}>
        isActive ? (
          <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" x2="12" y1="19" y2="22"/></svg>
        ) : (
